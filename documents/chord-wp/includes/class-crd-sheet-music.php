...
class CRD_Sheet_Music {
    ...
    public static function save_song_box( $post_id ) {
        ...
        if ( isset( $_POST['crd_save_song_nonce'] ) ) {
    ...
    public static function render_song( $content ) {
        ...
        if ( 'crd_sheet_music' ==  $post_type && is_main_query() ) {
            $location = get_post_meta( $post->ID, '_crd_song_location', true );
            ...
            // Make sure the filter is only run one time by removing it after the first run
            remove_filter( 'the_content', array( 'CRD_Sheet_Music', 'render_song' ) );
        }
